const businessIntroData = {
    ...
    "deltarobot": {
      ...
      description:
        ['델타 로봇(Delta Robot)은 병렬 링크 메커니즘(Parallel Kinematic Mechanism)을 기반으로 설계된 고속 고정밀 산업용 로봇입니다. 3개의 경량 링크가 삼각형 형태의 상단 프레임에 연결되어 있으며, 각각 독립적인 서보 모터에 의해 구동됩니다. 이 링크들은 공통된 플랫폼(엔드 이펙터)을 지지하며, 로봇의 공간 좌표를 고정밀하게 제어할 수 있도록 해줍니다. 델타 구조는 일반적인 직렬 구조 로봇에 비해 훨씬 빠른 응답성과 낮은 관성을 제공하므로, #초당 수백 회의 픽앤플레이스 작업을 요구하는 고속 생산 라인#라인에 특히 적합합니다. 이러한 구조적 이점 덕분에 #델타 로봇은 식음료, 제약, 전자부품, 화장품, 포장 산업 등 다양한 분야에서 널리 활용#되고 있습니다.', '#클린룸 인증(IP65 이상) 제품도 지원 가능#하므로, 위생 기준이 엄격한 환경에서도 안정적으로 작동하며, 생산 품질 향상 및 생산성 극대화에 기여할 수 있는 핵심 자동화 장비로 자리매김하고 있습니다.', 'DRF_ V 시리즈 차세대 플렉시블 워크스테이션은 CCD 모듈과 공급 모듈을 통합한 고정밀 델타 병렬 로봇을 탑재하여 #고정밀 트레이 배치, 분류 및 부착 작업을 수행#합니다. 전자, 하드웨어, 의료, 자동차, 반도체 등 다양한 산업 분야에서 널리 사용되고 있습니다. #모듈식 표준화 설계 컨셉을 채택하여 높은 호환성을 제공#하고, 비용 절감 및 효율성 향상, 작업 공간 축소, 교체 효율성 향상 등의 이점을 제공합니다.'],
      images: [`card`],
      link: {
        label: "자동화 솔루션 자세히 보기",
        path: "/automation-solution",
